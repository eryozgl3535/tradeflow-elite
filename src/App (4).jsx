...
function BenzerIsIpucu({baslik,jobs,edit,duzenlenecekId,T}){
  try{
    ...
    const benzer=jobs.filter(j=>j&&j.maliyet>0&&(!edit||j.id!==duzenlenecekId)&&kelimeler.some(w=>j.baslik&&j.baslik.toLowerCase().includes(w)));
...
function BenzerIsIpucu({baslik,jobs,edit,duzenlenecekId,T}){
  try{
    ...
    if(!benzer.length)return null;
    const ort=Math.round(benzer.reduce((s,j)=>s+(j.maliyet||0),0)/benzer.length/(KURLAR[AKTIF_PARA]||1));
    return <div style={{fontSize:11,color:P,fontWeight:600,marginTop:-8,marginBottom:12}}>💡 {T.benzerIs||"Benzer işlerde ort."}: {fmt(ort*(KURLAR[AKTIF_PARA]||1))} ({benzer.length} iş)</div>;
  }catch(e){return null;}
}
