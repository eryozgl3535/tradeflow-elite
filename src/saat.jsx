...
export function GokyuzuSahne({saat,dk=0,g=64,gen}){
  const dilim=gunDilimi(saat);
  const t=saat+dk/60;
  const W=gen||64, H=46;                 // panoramik için gen ver
  ...
  const cx=W*0.12+p*(W*0.76);
  const cy=44-Math.sin(p*Math.PI)*26;
  ...
  return <svg width={g} height={g*(H/W)} viewBox={`0 0 ${W} ${H}`} style={{display:"block",borderRadius:13,flexShrink:0}} aria-hidden="true">
    <defs>
      <linearGradient id={id} x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stopColor={gok[0]}/><stop offset="100%" stopColor={gok[1]}/>
      </linearGradient>
    </defs>
    <rect x="0" y="0" width={W} height={H} rx="9" fill={`url(#${id})`}/>

    {dilim==="gece"&&Array.from({length:Math.round(W/9)}).map((_,i)=>{
      const x=(i*17.3)%(W-6)+3, y=(i*11.7)%22+4;
      return <circle key={i} cx={x} cy={y} r={i%3===0?0.9:0.6} fill="#fff" opacity={0.5+((i*7)%35)/100}/>;
    })}

    {dilim!=="gece"&&<g opacity={dilim==="ogle"?0.85:0.62}>
      <ellipse cx={W*0.22} cy="13" rx={W*0.11} ry="3.2" fill="#ffffffcc"/>
      <ellipse cx={W*0.29} cy="12" rx={W*0.07} ry="2.5" fill="#ffffffcc"/>
      <ellipse cx={W*0.74} cy="20" rx={W*0.09} ry="2.6" fill="#ffffffa8"/>
    </g>}
    ...
    <path d={`M0 38 Q${W*0.25} 33 ${W*0.5} 37 Q${W*0.75} 41 ${W} 35 L${W} ${H} L0 ${H} Z`}
...
export function SelamSaat({ad,C,P,onDunya}){
  ...
  const gunduz=saat>=6&&saat<19;

  return <div style={{flex:1,minWidth:340,display:"flex",alignItems:"stretch",gap:0,background:C.card,border:`1px solid ${C.border}`,borderRadius:18,overflow:"hidden"}}>
    {/* Panoramik gökyüzü */}
    <div style={{position:"relative",flexShrink:0}}>
      <GokyuzuSahne saat={saat} dk={dk} g={168} gen={132}/>
      <div style={{position:"absolute",left:0,right:0,bottom:0,padding:"0 10px 7px",display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
        <span style={{fontSize:9,fontWeight:700,color:dilim==="gece"?"#94A9C4":"#5B4A33",letterSpacing:"0.05em"}}>
          {gunduz?"↑ 06:00":"↓ 19:00"}
        </span>
        <span style={{fontSize:9,fontWeight:700,color:dilim==="gece"?"#94A9C4":"#5B4A33",letterSpacing:"0.05em"}}>
          {gunduz?"↓ 19:00":"↑ 06:00"}
    ...
    {/* Metin bloğu */}
    <div style={{flex:1,minWidth:0,padding:"14px 20px",display:"flex",flexDirection:"column",justifyContent:"center"}}>
      <div style={{fontSize:24,fontWeight:800,color:C.t1,letterSpacing:"-0.025em",lineHeight:1.15}}>
        {d.selam}{ad?", "+ad:""}
      </div>
      <div style={{fontSize:12.5,color:C.t3,marginTop:3,marginBottom:9}}>{d.alt}</div>

      <div style={{display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
        <span style={{fontSize:21,fontWeight:800,color:C.t1,fontVariantNumeric:"tabular-nums",letterSpacing:"-0.02em",lineHeight:1}}>
          {ss}:{mm}<span style={{fontSize:13,color:C.t3,fontWeight:700}}>:{sn}</span>
        </span>
        <span style={{width:1,height:17,background:C.border,display:"block"}}/>
